...
int check_login(const char* u, const char* p) {
    return (strcmp(u,"admin")==0 && strcmp(p,"1234")==0) || (strcmp(u,"user")==0 && strcmp(p,"pass")==0);
}
...
void execute_service(int fd, int code, time_t start) {
    ...
    if (code == 1) {
        ...
        if (n <= 0) { write(fd, "Aucun nom reçu\n", 14); return; }
        buffer[n]=0;

        char *newline = strchr(buffer, '\n'); if (newline) *newline=0;
        FILE *f = fopen(buffer, "r");
        if (!f) { write(fd, "Impossible d'ouvrir le fichier\n", 31); return; }
